/** @file
  ...
  @authors Jakub Pawlewicz <pan@mimuw.edu.pl>, Marcin Peczarski <marpe@mimuw.edu.pl>, Wiktoria Walczak
...
/**
 * Sprawdza, czy jednomian jest równy zeru
 * @param[in] m : jednomian
...
static inline bool PolyIsZero(const Poly *p) {
    if (p->arr == NULL) {
        ...
        if ((p->size == 1) && (PolyIsZero(&p->arr[0].p))) {
            return true;
        }
        return false;
    }
 ...
 */
static inline Poly PolyClone(const Poly *p) {
    Poly new;
    if (p->arr == NULL) {
        return PolyFromCoeff(p->coeff);
    }
    new.size = p->size;
    Mono *arr = malloc(new.size * sizeof *arr);
...
/**
 * Sprawdza czy wielomian ma wyraz wolny, czyli jednomian o wykładniku równym zeru.
 * @param[in] p : wielomian @f$p@f$
 * @return Czy wielomian ma wyraz wolny?
 */
static inline bool PolyHasExpZero(const Poly *p) {
    return p->arr[0].exp == 0;
}

/**
 * Funkcja realizująca składanie wielomianów.
